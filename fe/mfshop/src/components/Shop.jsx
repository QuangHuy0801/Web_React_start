...
const Shop = () => {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        ...
        setFilteredProducts(allProducts); // Đặt filteredProducts mặc định là tất cả sản phẩm
      } catch (error) {
        console.error(error);
  ...
  useEffect(() => {
    if (categoryId) {
      const categoryIdInt = parseInt(categoryId, 10); // Chuyển đổi categoryId sang kiểu int
